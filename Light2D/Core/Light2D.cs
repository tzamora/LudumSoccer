...
public class Light2D : MonoBehaviour
{
    ...
    void CollectColliders()
    {
        ...
        else
        {
            ...
            objs._2DColliders = Physics2D.OverlapAreaAll(transform.TransformPoint(DiectionalLightPivotPoint + transform.position + new Vector3(-beamSize, beamRange, 0)), transform.TransformPoint(DiectionalLightPivotPoint + new Vector3(beamSize, -beamRange, 0)), shadowLayer);
        }
    ...
    void UpdateRadialUVs()
    {
        ...
        for (int i = 0; i < verts.Count; i++)
        {
            uvs.Add(new Vector2((verts[i].x * 0.5f) / (lightRadius * uvTiling.x) + (0.5f + uvOffset.x), (verts[i].y * 0.5f) / (lightRadius * uvTiling.y) + (0.5f + uvOffset.y)));
        }
